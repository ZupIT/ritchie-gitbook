...
When a user will download Ritchie \([Single](../../../geting-started/software-architecture-1/single-version.md) version\) or execute the `rit login` command \([Team](../../../geting-started/software-architecture-1/team-version.md) version\) to access an organization's repositories, the CLI will download and merge the tree.json from the formula repositories to which the user has access.
